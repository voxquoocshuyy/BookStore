 ...
 * @author Quoc Huy
 */
@WebServlet(name = "AdminActionController", urlPatterns = {"/AdminActionController"})
public class AdminActionController extends HttpServlet {

    private static final String ERROR = "error.html";
    private static final String SUCCESS = "adminUpdate.jsp";
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            ProductDTO book = dao.getBookByID(bookID);
            request.setAttribute("BOOK_UPDATE", book);
            CategoryDAO daoCate = new CategoryDAO();
            List<CategoryDTO> list = daoCate.getAll();
            request.setAttribute("LIST_CATE", list);
            url = SUCCESS;
        } catch (Exception e) {
            log("Error at AdminActionController: " + e.getMessage());
        }finally{
