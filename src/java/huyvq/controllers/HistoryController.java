    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (session != null) {
                ...
                String temp = request.getParameter("txtDateOrder");
                if (temp == null || temp == "") {
                    OrderDAO dao = new OrderDAO();
                    List<OrderDTO> list = dao.getAll(userID);
                    request.setAttribute("HISTORY", list);
                    url = SUCCESS;
                } else {
                    Date dateOrder = Date.valueOf(temp);
                    OrderDAO dao = new OrderDAO();
                    List<OrderDTO> list = dao.getAllDate(dateOrder, userID);
                    request.setAttribute("HISTORY", list);
                    url = SUCCESS;
                }
            }
